...
int main (int argc, char** argv)
{
    ...
    //Fill the two input arrays with random data
    srandom((unsigned int)time(NULL));
    for (int i = 0; i < DATA_SIZE; ++i)
    {
        ((float*)Ia->data)[i] = (float)random()/RAND_MAX;
        ((float*)Ib->data)[i] = (float)random()/RAND_MAX;
    }

    //Create the instruction (ADD)
    cphvb_instruction inst;
    inst.opcode = CPHVB_ADD;
    inst.operand[0] = R;
    inst.operand[1] = Ia;
    inst.operand[2] = Ib;
    ...
    {    //Tell the VEM to perform the instruction.
        ...
        if(error != CPHVB_SUCCESS)
        {
            printf("Error executing ADD instruction\n");
            exit(-1);
        } 
    } 
