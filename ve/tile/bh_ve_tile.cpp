...
static bh_intp bh_ve_tile_buffersizes = 0;
static bh_computeloop_naive* bh_ve_tile_compute_loops = NULL;
static bh_tstate_naive* bh_ve_tile_tstates = NULL;
...
bh_error bh_ve_tile_init(bh_component *self)
{
    myself = self;                              // Assign config container.

    char *env = getenv("BH_VE_TILE_BLOCKSIZE");   // Override block_size from environment-variable.
    if(env != NULL)
    {
        block_size = atoi(env);
    }
    ...
    {
        fprintf(stderr, "BH_VE_TILE_BLOCKSIZE (%ld) should be greater than zero!\n", (long int)block_size);
        return BH_ERROR;
    }

    env = getenv("BH_VE_TILE_BINMAX");      // Override block_size from environment-variable.
    if(env != NULL)
    {
        bin_max = atoi(env);
    }
    ...
    {
        fprintf(stderr, "BH_VE_TILE_BINMAX (%ld) should be greater than zero!\n", (long int)bin_max);
        return BH_ERROR;
    }

    env = getenv("BH_VE_TILE_BASEMAX");      // Override base max from environment-variable.
    if(env != NULL)
    {
        base_max = atoi(env);
    }
    ...
    {
        fprintf(stderr, "BH_VE_TILE_BASEMAX (%ld) should be greater than zero!\n", (long int)base_max);
        return BH_ERROR;
    }
    ...
    //printf("[BH_VE_TILE_BLOCKSIZE=%ld]\n", block_size);
    //printf("[BH_VE_TILE_BINMAX=%ld]\n", bin_max);
    //printf("[BH_VE_TILE_BASEMAX=%ld]\n", base_max);
...
inline bh_error block_execute( bh_instruction* instr, bh_intp start, bh_intp end) {
    ...
    if ((end - start + 1) > bh_ve_tile_buffersizes) // Make sure we have enough space
    {
    	...
    	bh_ve_tile_buffersizes = 0;                 // Make sure we re-allocate on error
    	
    	if (bh_ve_tile_compute_loops != NULL) {
    		free(bh_ve_tile_compute_loops);
    		free(bh_ve_tile_tstates);
    		bh_ve_tile_compute_loops = NULL;
    		bh_ve_tile_tstates = NULL;
    	}
    	
    	bh_ve_tile_compute_loops    = (bh_computeloop_naive*)malloc(sizeof(bh_computeloop_naive) * mcount);
        bh_ve_tile_tstates          = (bh_tstate_naive*)malloc(sizeof(bh_tstate_naive)*mcount);
    	
    	if (bh_ve_tile_compute_loops == NULL)
    		return BH_OUT_OF_MEMORY;
    	if (bh_ve_tile_tstates == NULL)
    		return BH_OUT_OF_MEMORY;
    	
    	bh_ve_tile_buffersizes = mcount;
    }
    
    bh_computeloop_naive* compute_loops = bh_ve_tile_compute_loops;
    bh_tstate_naive* states = bh_ve_tile_tstates;
...
bh_error bh_ve_tile_execute( bh_intp instruction_count, bh_instruction* instruction_list )
{
    bh_intp cur_index,  j;
...
bh_error bh_ve_tile_shutdown( void )
{
    bh_vcache_clear();
    bh_vcache_delete();
...
bh_error bh_ve_tile_reg_func(char *fun, bh_intp *id) {
