...
# Let's find the version as a tuple e.g. v0.8.9-47-g6464 => (0, 8, 9, 47)
try:
    cmd = "git describe --tags --long --match v[0-9]*"
    version_str = str(subprocess.check_output(cmd, shell=True, cwd=src_root()).decode('utf-8')).strip()
...
# In order to convert the version string to a tuple, we match major, minor, micro, and post version.
version_info = [0] * 4
m = re.match("v?(\d+)", version_str)
if m:
    version_info[0] = int(m.group(1))
m = re.match("v?\d+\.(\d+)", version_str)
if m:
    version_info[1] = int(m.group(1))
m = re.match("v?\d+\.\d+\.(\d+)", version_str)
if m:
    version_info[2] = int(m.group(1))
m = re.match("v?[\d\.]+\-(\d+)", version_str)
if m:
    version_info[3] = int(m.group(1))
version_info = tuple(version_info)

version_str = "%d.%d.%d" % (version_info[0], version_info[1], version_info[2])
if version_info[3] > 0:
    version_str += ".post%d" % version_info[3]

print("Bohrium API version: %s" % version_str)


def write_version(o):
    """Write the version.py file to `o`"""
    o.write("__version__ = \"%s\"\n" % version_str)
    o.write("__version_info__ = (%d, %d, %d, %d)\n" % version_info)
