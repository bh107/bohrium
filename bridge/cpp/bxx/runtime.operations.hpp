...
// Explicit result array
template <typename OutT, typename InT>
multi_array<OutT>& bh_scatter (multi_array<OutT>& res, multi_array<InT>& lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, OutT, InT, InT>();
...
template <typename OutT, typename InT>
multi_array<OutT>& bh_scatter (multi_array<OutT>& res, multi_array<InT>& lhs, const InT rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, OutT, InT, InT>();
    ...
    if (res.getRank() < left->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_scatter: " << res.getRank() << ", " << left->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
    ...
    Runtime::instance().enqueue((bh_opcode)BH_SCATTER, res, *left, rhs);
    return res;
}

template <typename OutT, typename InT>
multi_array<OutT>& bh_scatter (multi_array<OutT>& res, const InT lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, OutT, InT, InT>();
    ...
    if (res.getRank() < right->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_scatter: " << res.getRank() << ", " << right->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
    ...
    Runtime::instance().enqueue((bh_opcode)BH_SCATTER, res, lhs, *right);
    return res;
}
...
//
template <typename T>
multi_array<T>& bh_scatter (multi_array<T> &lhs, multi_array<T> &rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, T, T, T>();
...
template <typename T>
multi_array<T>& bh_scatter (multi_array<T>& lhs, const T rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, T, T, T>();
    ...
    Runtime::instance().enqueue((bh_opcode)BH_SCATTER, *result, lhs, rhs);
    return *result;
}

template <typename T>
multi_array<T>& bh_scatter (const T lhs, multi_array<T>& rhs)
{
    Runtime::instance().typecheck<BH_SCATTER, T, T, T>();
...
// Explicit result array
template <typename OutT, typename InT>
multi_array<OutT>& bh_gather (multi_array<OutT>& res, multi_array<InT>& lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_GATHER, OutT, InT, InT>();
...
template <typename OutT, typename InT>
multi_array<OutT>& bh_gather (multi_array<OutT>& res, multi_array<InT>& lhs, const InT rhs)
{
    Runtime::instance().typecheck<BH_GATHER, OutT, InT, InT>();
    ...
    if (res.getRank() < left->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_gather: " << res.getRank() << ", " << left->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
    ...
    Runtime::instance().enqueue((bh_opcode)BH_GATHER, res, *left, rhs);
    return res;
}

template <typename OutT, typename InT>
multi_array<OutT>& bh_gather (multi_array<OutT>& res, const InT lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_GATHER, OutT, InT, InT>();
    ...
    if (res.getRank() < right->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_gather: " << res.getRank() << ", " << right->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
    ...
    Runtime::instance().enqueue((bh_opcode)BH_GATHER, res, lhs, *right);
    return res;
}
...
//
template <typename T>
multi_array<T>& bh_gather (multi_array<T> &lhs, multi_array<T> &rhs)
{
    Runtime::instance().typecheck<BH_GATHER, T, T, T>();
...
template <typename T>
multi_array<T>& bh_gather (multi_array<T>& lhs, const T rhs)
{
    Runtime::instance().typecheck<BH_GATHER, T, T, T>();
    ...
    Runtime::instance().enqueue((bh_opcode)BH_GATHER, *result, lhs, rhs);
    return *result;
}

template <typename T>
multi_array<T>& bh_gather (const T lhs, multi_array<T>& rhs)
{
    Runtime::instance().typecheck<BH_GATHER, T, T, T>();
...
// Explicit result array
template <typename OutT, typename InT>
multi_array<OutT>& bh_matmul (multi_array<OutT>& res, multi_array<InT>& lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, OutT, InT, InT>();
...
template <typename OutT, typename InT>
multi_array<OutT>& bh_matmul (multi_array<OutT>& res, multi_array<InT>& lhs, const InT rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, OutT, InT, InT>();
    ...
    if (res.getRank() < left->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_matmul: " << res.getRank() << ", " << left->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
...
template <typename OutT, typename InT>
multi_array<OutT>& bh_matmul (multi_array<OutT>& res, const InT lhs, multi_array<InT>& rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, OutT, InT, InT>();
    ...
    if (res.getRank() < right->getRank()) {
        std::stringstream s;
        s << "Incompatible shapes " << "bh_matmul: " << res.getRank() << ", " << right->getRank() << "." << std::endl;
        throw std::runtime_error(s.str());
    }
...
//
template <typename T>
multi_array<T>& bh_matmul (multi_array<T> &lhs, multi_array<T> &rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, T, T, T>();
...
template <typename T>
multi_array<T>& bh_matmul (multi_array<T>& lhs, const T rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, T, T, T>();
...
template <typename T>
multi_array<T>& bh_matmul (const T lhs, multi_array<T>& rhs)
{
    Runtime::instance().typecheck<BH_MATMUL, T, T, T>();
