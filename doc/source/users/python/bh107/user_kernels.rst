...
In order to write and run your own kernel use :func:`bh107.user_kernel.execute()`::
...
Two useful help functions when writing user kernels is :func:`bh107.user_kernel.make_behaving()`, which makes that an array is of a specific data type, is contiguous, and uses no offset and :func:`bh107.user_kernel.dtype_to_c99()`, which converts a Bh107/NumPy array data type into a C99 data type.


OpenCL Example
--------------

In order to use the OpenCL backend, use the `tag` and `param` of :func:`bh107.user_kernel.execute()`::
