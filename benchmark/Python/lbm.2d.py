...
t       = np.array([4/9., 1/9.,1/9.,1/9.,1/9., 1/36.,1/36.,1/36.,1/36.], dtype=float, bohrium=B.bohrium)
t_l     = np.array([4/9., 1/9.,1/9.,1/9.,1/9., 1/36.,1/36.,1/36.,1/36.], dtype=float, bohrium=B.bohrium)
cx      = np.array([  0,   1,  0, -1,  0,    1,  -1,  -1,   1], dtype=float, bohrium=B.bohrium)
cx_l    = np.array([  0,   1,  0, -1,  0,    1,  -1,  -1,   1], dtype=float, bohrium=B.bohrium)
cy      = np.array([  0,   0,  1,  0, -1,    1,   1,  -1,  -1], dtype=float, bohrium=B.bohrium)
cy_l    = np.array([  0,   0,  1,  0, -1,    1,   1,  -1,  -1], dtype=float, bohrium=B.bohrium)
opp     = np.array([ 0,   3,  4,  1,  2,    7,   8,   5,   6], dtype=float, bohrium=B.bohrium)
col     = np.array(xrange(2,ly), dtype=float, bohrium=B.bohrium)

bbRegion        = np.empty((lx,ly), dtype=float, bohrium=B.bohrium)
not_bbRegion    = np.empty((lx,ly), dtype=float, bohrium=B.bohrium)
...
fIn = np.empty([9,lx,ly], dtype=float, bohrium=B.bohrium)
fIn[:] = 1.0 * t[:,np.newaxis,np.newaxis]
fEq = np.empty([9,lx,ly], dtype=float, bohrium=B.bohrium)
fEq[:] = 1.0
fOut = np.empty([9,lx,ly], dtype=float, bohrium=B.bohrium)
fOut[:] = 1.0
...
def lbm2d():
    ...
    for cycle in xrange(maxT):
        ...
        uy[0,2:] *= 0
        t1 = np.empty(fIn[0:5:2,0,2:].shape, dtype=float, bohrium=B.bohrium)
        t2 = np.empty(fIn[3:8:2,0,2:].shape, dtype=float, bohrium=B.bohrium)
        t1[:] = fIn[0:5:2,0,2:]
        t2[:] = fIn[3:8:2,0,2:]
        ...
        for i in xrange(0, 9):
            ...
            # Bounce back region:
            BB = np.empty((lx,ly), bohrium=B.bohrium)
            BB[:] = fIn[opp[i]]
            BB *= bbRegion
            fOut[i] *= not_bbRegion
            fOut[i] += BB
        ...
        for i in xrange(0,9):
            if cx_l[i] == 1:
                t1 = np.empty(fOut[i].shape, dtype=float, bohrium=B.bohrium)
                t1[1:] = fOut[i][:-1]
                t1[0] = fOut[i][-1]
                fOut[i] = t1
            elif cx_l[i] == -1:
