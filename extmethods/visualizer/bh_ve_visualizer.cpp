...
static bh_component_iface *child;   // Function pointers to our child.
...
/*
    ...
    for(i=0; i<(int)batch.size(); ++i) {
        ...
        if (i >= (int)batch.size()) {  // No visualize instruction
            break;
        }
        ...
        if (size > 0) {                  // Execute up until the visualize
            bh_ir new_bhir;
        ...
        bh_ir new_bhir;                 // Execute the visualize instruction after a SYNC
    ...
    bh_intp size = i-start;             // Execute the remaining instructions in the batch
    if(size > 0) {
        bh_ir new_bhir;
        bh_error e = bh_ir_create(&new_bhir, size, &batch[start]);
        if(e != BH_SUCCESS) {
...
bh_error bh_ve_visualizer_execute(bh_ir* bhir)
{
    for(krnl_iter krnl = bhir->kernel_list.begin();
        ...
        krnl++) {
        // Send kernels to child that does not contain visualizer_opcode

        // Execute kernels containing visualizer_opcode
