...
void ExecuteFrontend::serialize(bh_ir &bhir, vector<char> &buffer, vector<bh_base *> &data_send,
    ...
    for(size_t i=0; i<bhir.instr_list.size(); ++i) {
        ...
        for (const bh_view &v: instr.operand) {
            ...
            if (not util::exist(known_base_arrays, v.base)) {
                // Erase sync instructions that syncs new base arrays
...
bh_ir ExecuteBackend::deserialize(vector<char> &buffer, vector<bh_base *> &data_send, vector<bh_base *> &data_recv) {
    ...
    for (const bh_instruction &instr: bhir.instr_list) {
        ...
        switch (instr.opcode) {
            case BH_SYNC: {
                bh_data_malloc(instr.operand[0].base);
