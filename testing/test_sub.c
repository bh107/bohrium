...
int main (int argc, char** argv)
{
    ...
    cphvb_instruction inst;             //Create the instruction (SUBTRACT)
    inst.opcode = CPHVB_SUBTRACT;
    inst.operand[0] = R;
    inst.operand[1] = Ia;
    inst.operand[2] = Ib;
    ...
    {                                   //Tell the VEM to perform the instruction.
        ...
        if(error != CPHVB_SUCCESS)
        {
            printf("Error executing SUBTRACT instruction\n");
            exit(-1);
        } 
    } 
    else                                // We should do it our selves 
    {                                   // This is left as an exercise ;-)
        printf("Throwing up: Operation SUBTRACT not supported\n");
        exit(-1);
    }
    ...
    {
        float sub = ((float*)Ia->data)[i] - ((float*)Ib->data)[i]; 
        if (((float*)R->data)[i] != 0.0 )
        {
            success = 0;
            break;
        }
