...
int main (int argc, char** argv)
{
    ...
    cphvb_instruction inst;                             // Create the instruction (MINIMUM)
    inst.opcode = CPHVB_MINIMUM;
    inst.operand[0] = R;
    inst.operand[1] = Ia;
    inst.operand[2] = Ib;
    ...
    if (vem_instruction_check(&inst)) {                 //  Tell the VEM to perform the instruction.
        ...
        printf("Throwing up: Operation ADD not supported\n");
        exit(-1);
    }
