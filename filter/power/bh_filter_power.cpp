...
bh_error bh_filter_power_init(bh_component *self)
{
...
bh_error bh_filter_power_execute(bh_ir* bhir)
{
    power_filter(bhir);                 // Run the filter
    return child_execute(bhir);             // Execute the filtered bhir
}

bh_error bh_filter_power_shutdown(void)
{
    bh_error ret;
...
bh_error bh_filter_power_reg_func(const char *fun, bh_intp *id)
{
