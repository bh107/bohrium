...
bh_error bh_filter_validate_init(const char* name)
{
    bh_error err;
...
bh_error bh_filter_validate_shutdown(void)
{
...
bh_error bh_filter_validate_execute(bh_ir* bhir)
{
    validate_filter(bhir);        // Run the filter
    return child->execute(bhir); // Execute the filtered bhir
}

bh_error bh_filter_validate_extmethod(const char *name, bh_opcode opcode)
{
