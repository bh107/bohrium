...
bh_error bh_filter_graph_init(const char* name)
{
    bh_error err;
    ...
    {
        fprintf(stderr, "[graph-FILTER] Unexpected number of children, must be 1");
        return BH_ERROR;
    }
...
bh_error bh_filter_graph_shutdown(void)
{
...
bh_error bh_filter_graph_execute(bh_ir* bhir)
{
    graph_filter(bhir);        // Run the filter
    return child->execute(bhir); // Execute the filtered bhir
}

bh_error bh_filter_graph_extmethod(const char *name, bh_opcode opcode)
{
