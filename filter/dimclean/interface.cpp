...
bh_error bh_filter_dimclean_init(const char* name)
{
    bh_error err;
    ...
    {
        fprintf(stderr, "[DIMCLEAN-FILTER] Unexpected number of children, must be 1");
        return BH_ERROR;
    }

    timing = bh_component_config_lookup_bool(&myself, "timing", 0);
    if (timing)
        exec_timing = bh_timer_new("[DimClean] Execution");
...
bh_error bh_filter_dimclean_shutdown(void)
{
...
bh_error bh_filter_dimclean_extmethod(const char *name, bh_opcode opcode)
{
