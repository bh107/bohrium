...
bh_error bh_filter_pricer_init(const char* name)
{
    bh_error err;
    ...
    {
        fprintf(stderr, "[PRICER-FILTER] Unexpected number of children, must be 1");
        return BH_ERROR;
    }
...
bh_error bh_filter_pricer_execute(bh_ir* bhir)
{
    filter(*bhir);        // Run the filter
    return child->execute(bhir); // Execute the filtered bhir
}

bh_error bh_filter_pricer_extmethod(const char *name, bh_opcode opcode)
{
