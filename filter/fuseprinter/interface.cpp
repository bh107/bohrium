...
bh_error bh_filter_fuseprinter_init(const char* name)
{
    bh_error err;
...
bh_error bh_filter_fuseprinter_shutdown(void)
{
...
bh_error bh_filter_fuseprinter_execute(bh_ir* bhir)
{
    filter(*bhir);        // Run the filter
    return child->execute(bhir); // Execute the filtered bhir
}

bh_error bh_filter_fuseprinter_extmethod(const char *name, bh_opcode opcode)
{
