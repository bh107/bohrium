...
bh_error bh_fuser_topological_init(const char* name)
{
    bh_error err;
    ...
    {
        fprintf(stderr, "[TOPOLOGICAL FUSER] Unexpected number of children, must be 1");
        return BH_ERROR;
    }

    timing = bh_component_config_lookup_bool(&myself, "timing", false);
    if (timing)
        exec_timing = bh_timer_new("[Topological fuser] Execution");
...
bh_error bh_fuser_topological_shutdown(void)
{
...
bh_error bh_fuser_topological_extmethod(const char *name, bh_opcode opcode)
{
