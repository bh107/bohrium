...
bh_error bh_fuser_optimal_init(const char* name)
{
    bh_error err;
    ...
    {
        fprintf(stderr, "[OPTIMAL FUSER] Unexpected number of children, must be 1");
        return BH_ERROR;
    }

    timing = bh_component_config_lookup_bool(&myself, "timing", false);
    if (timing)
        exec_timing = bh_timer_new("[Optimal fuser] Execution");
...
bh_error bh_fuser_optimal_shutdown(void)
{
...
bh_error bh_fuser_optimal_execute(bh_ir* bhir)
{
    bh_uint64 start = 0;
    if (timing)
...
bh_error bh_fuser_optimal_extmethod(const char *name, bh_opcode opcode)
{
